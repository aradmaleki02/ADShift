...
import torch
from dataset import get_data_transforms, BrainTest, Waterbird
...
def test_mvtec(_class_, epoch, backbone, image_size, cp_path):
    ...
    test_data1 = Waterbird(root='/kaggle/input/waterbird/waterbird', df=df, transform=data_transform,
                                 train=False, count_train_landbg=3500, count_train_waterbg=100, mode='bg_land')
    test_data2 = Waterbird(root='/kaggle/input/waterbird/waterbird', df=df, transform=data_transform,
    ...
    auroc_sp = evaluation_ATTA(encoder, bn, decoder, test_dataloader1, device,
                               ...
                               img_size=image_size, lamda=lamda, dataset_name='waterbirds', _class_=_class_)
